...
class NFI5MOHO_WIP(IStrategy):
    ...
    # Custom Stoploss params - time based
    custom_stop_initial = DecimalParameter(-0.15, -0.10, default=-0.15, space='sell', decimals=3, optimize=False, load=True)
    custom_stop_ramp_minutes = IntParameter(5, 70, default=50, space='sell', optimize=False, load=True)
    custom_stop_ramp_rate = DecimalParameter(0.01, 0.02, default=0.01, space='sell', decimals=3, optimize=False, load=True)
    custom_stop_ramp_max = DecimalParameter(0.01, 0.06, default=0.01, space='sell', decimals=3, optimize=False, load=True)

    # Custom Stoploss params - profit based
    custom_stop_profit_1 = DecimalParameter(0.01, 0.04, default=0.01, space='sell', decimals=3, optimize=False, load=True)
    custom_stop_profit_2 = DecimalParameter(0.02, 0.08, default=0.02, space='sell', decimals=3, optimize=False, load=True)
    custom_stop_profit_3 = DecimalParameter(0.03, 0.12, default=0.03, space='sell', decimals=3, optimize=False, load=True)
    
    custom_stoploss_1 = DecimalParameter(-0.05, -0.01, default=-0.03, space='sell', decimals=3, optimize=False, load=True)
    custom_stoploss_2 = DecimalParameter(-0.03, -0.01, default=-0.02, space='sell', decimals=3, optimize=False, load=True)
    custom_stoploss_3 = DecimalParameter(-0.02, -0.01, default=-0.01, space='sell', decimals=3, optimize=False, load=True)
    ...
    def custom_stoploss(self, pair: str, trade: 'Trade', current_time: datetime,
        ...
        Trading happens on 5m timeframe, so 1 candle = 5 minutes
        """
        
        # Get the minutes elapsed since trade opened
        minutes_elapsed = (current_time - trade.open_date_utc).total_seconds() / 60
        
        # Time-based stoploss
        current_stoploss = self.custom_stop_initial.value
        
        # Increase stoploss based on time
        stoploss_adjustment = min(
            self.custom_stop_ramp_max.value,
            (minutes_elapsed / self.custom_stop_ramp_minutes.value) * self.custom_stop_ramp_rate.value
        )
        
        # Apply time-based adjustment
        dynamic_stoploss = current_stoploss + stoploss_adjustment

        # Profit-based stoploss
        if current_profit > self.custom_stop_profit_3.value:
            # Strong profits, very tight stoploss
            dynamic_stoploss = max(dynamic_stoploss, self.custom_stoploss_3.value)
        elif current_profit > self.custom_stop_profit_2.value:
            # Good profits, moderately tight stoploss
            dynamic_stoploss = max(dynamic_stoploss, self.custom_stoploss_2.value)
        elif current_profit > self.custom_stop_profit_1.value:
            # Small profits, lighter stoploss
            dynamic_stoploss = max(dynamic_stoploss, self.custom_stoploss_1.value)
        
        # Ensure we don't go above 0 (which would be a guaranteed loss)
        return max(dynamic_stoploss, -0.01)
    ...
    limit_sell_timeout = 60  # 60 seconds for limit order attempt
    _last_sell_timestamps = {}  # Track sell order times
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Track if we should use market order
        current_time = datetime.now(timezone.utc)
        pair = metadata['pair']
        
        if pair in self._last_sell_timestamps:
            time_since_first_sell = (current_time - self._last_sell_timestamps[pair]).total_seconds()
            
            # If we've waited more than limit_sell_timeout seconds, switch to market
            if time_since_first_sell > self.limit_sell_timeout:
                dataframe.loc[:, 'sell_type'] = 'market'
                # Clean up timestamp
                del self._last_sell_timestamps[pair]
            else:
                dataframe.loc[:, 'sell_type'] = 'limit'
        else:
            # First sell attempt - use limit order
            dataframe.loc[:, 'sell_type'] = 'limit'
            self._last_sell_timestamps[pair] = current_time
    ...
    def confirm_trade_exit(self, pair: str, trade, order_type: str, amount: float,
        ...
        Allows checking if placing a sell order is a good idea.
        """
        # Track first sell attempt timestamp
        if pair not in self._last_sell_timestamps:
            self._last_sell_timestamps[pair] = current_time
            
        return True

    def check_exit_timeout(self, pair: str, trade, order, current_time) -> bool:
        """
        Called when checking if a trade should be exited.
        This is called when an order is cancelled or expired.
        """
        # If we've exceeded our timeout, switch to market order
        if pair in self._last_sell_timestamps:
            time_since_first_sell = (current_time - self._last_sell_timestamps[pair]).total_seconds()
            if time_since_first_sell > self.limit_sell_timeout:
                # Clean up timestamp
